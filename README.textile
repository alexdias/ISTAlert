...
Version 3 addresses the most recent changes in Twitter's authentication requirements: the need to use OAuth. Given that this is a single-user application, we can simply use a single access token, which will sign the requests done to Twitter's API. This is done using the OAuth ruby gem: http://oauth.rubyforge.org/
There is also a python version included for systems that prefer to use python instead of ruby. This version uses the punyURL service from services.sapo.pt instead of the original GET request to http://puny.sapo.pt that was present in previous versions.
The python version depends on the OAuth lib by Brian Rosner. You can find it at https://github.com/brosner/python-oauth2
